...
import { MapPin, ArrowRight, Sparkles, Search, ChevronDown } from 'lucide-react';
...
export const HeroSection: React.FC<HeroSectionProps> = ({ onSearch }) => {
  ...
  return (
    <header className="relative w-full h-[calc(100vh)] min-h-[660px] max-h-[960px] flex flex-col justify-between pt-24 pb-8 md:pb-12 px-6 md:px-16 overflow-hidden">
      ...
      {/* Stylish Rounded Floating Search Bar (Mysore Localized) */}
      <div className="relative z-20 w-full max-w-[1280px] mx-auto animate-fade-in-up animate-delay-1 mt-4 sm:mt-8">
        <form
          onSubmit={handleSubmit}
          className="bg-black/45 dark:bg-black/55 backdrop-blur-2xl border border-white/30 rounded-full p-2 sm:p-2.5 w-full max-w-3xl shadow-[0_20px_50px_rgba(0,0,0,0.35)] flex flex-col sm:flex-row items-center gap-2 sm:gap-3 transition-all duration-300 hover:border-white/50"
        >
          ...
          <div className="w-full sm:w-auto flex items-center pl-4 sm:pl-2 pr-4 py-2 relative">
            <select
              value={type}
          ...
          <button
            type="submit"
            className="w-full sm:w-auto bg-[#a2650c] hover:bg-[#8c5208] text-white px-7 py-3 rounded-full transition-all duration-300 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 flex-shrink-0 shadow-lg hover:scale-105 active:scale-95"
          >
      ...
      {/* Hero Narrative & CTA: New Flagship Headline */}
      <div className="relative z-20 w-full max-w-[1280px] mx-auto text-white animate-fade-in-up animate-delay-2">
        ...
        <div>
          <a
            ...
            className="group inline-flex items-center gap-3 bg-[#fed488] text-[#785a1a] px-8 py-3.5 font-sans text-xs font-bold uppercase tracking-[0.18em] rounded-full hover:bg-white hover:text-black transition-all duration-300 shadow-2xl hover:-translate-y-0.5"
          >
